...
int main(int argc, char *argv[])
{
				...
				else if (fGetGPS) { //FIXME: check if fFalseGPS is set to display some advertising
								gps_get_data();     // Save file with contents of register 
				}
...
void *thread_isr(void *p) 
{
				...
				while(1)
								if (wait_for_interrupt(intc_fd, intc_ptr)){
												...
												//FIXME: see how to write a float to uint32_t
												dev_write(cfg_ptr,CFG_TIME_OFFSET, gps_data.times);
												dev_write(cfg_ptr,CFG_LATITUDE_OFFSET, gps_data.latitude);
												dev_write(cfg_ptr,CFG_LONGITUDE_OFFSET, gps_data.longitude);
												dev_write(cfg_ptr,CFG_ALTITUDE_OFFSET, gps_data.altitude);
												dev_write(cfg_ptr,CFG_SATELLITE_OFFSET, gps_data.satellites);
